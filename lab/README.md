...
One continuous world, never a cut. On PLAY the title board is hauled back up
its own ropes and out of frame, the button drops away beneath it, the camera
settles on the grass, and she simply keeps walking — so the instruction sign
that arrives a moment later reads as the NEXT sign on the same ropes rather
than as a new screen. The departure overlaps her walk-in deliberately: the eye
never sees an empty stage between the two.

Both the arrival and the departure animations are set inline, because an
inline style outranks a class rule and the two would otherwise fight.
...
- **rail-v.png** is the depth run, cut from the supplied depth-fence art
  ...
  one timber.
- **area-card.png** is the supplied score board (`score.png`), cropped once to
  its opaque box. Its inner panel was measured off the artwork, so the area
  number and the recap figures sit exactly on the wood. The recap uses it too:
  cream text on cartoon grass is hard to read, so each build's figures sit on
  a board.
- **touch.png** is the supplied `touch.svg` hand. That file carries three
  3000px PNGs and weighs 13.6 MB; it is rasterised once to a 29 KB asset at
  the size it is actually drawn, recoloured from its dull mauve-and-peach into
  the game's own cream and button orange, and mirrored IN THE ARTWORK so it
  still points at the post when animation is switched off.
- **sign.png** is the supplied panel art (`pannel.png`), cropped once to its
  opaque box and otherwise untouched. It is NOT sliced or tiled: the straw is
  woven on a 75px period, the ropes exist only at the ends, and the board is
  not centred in its own frame - so any reconstruction shows. The sign is used
  whole at its true aspect, and its ropes are clipped by the top of the STAGE
  rather than by a crop, which is how a sign hanging from off-screen reads
  anyway. `fitSign()` steps the type down if a line would ever outrun the
  board, so nothing can wrap or overflow it.
...
This folder IS the deployed game. The repo root is a Vercel static site: `/` is
a launcher, `/game` is this, and `/lab` is a duplicate to experiment in. There
is no build and no publish step - edit here and deploy. See `DEPLOY.md`.
