...
Object.assign(FenceTheFarm.prototype, {
  ...
  /* The background bed. `vol` is the ceiling it is ever allowed to reach, and
     it is low on purpose: this track sits under the game, not over it.
     `duck` is the fraction it drops to when something else needs to be heard,
     and the two `Ms` values are how long it stays out of the way. */
  BGM: { vol: 0.13, duck: 0.40, duckMs: 480, voDuck: 0.24, voMs: 1700, fadeIn: 2200 },
  /* Sounds loud or important enough that the bed should step aside. Fence
     construction patter and the tiny UI ticks are deliberately absent: ducking
     on those would make the bed pump continuously while a fence is built. */
  BGM_DUCK: {
    measure_tick: 1, success_chord: 1, record_break: 1, record_success: 1,
    fence_snap_big: 1, challenge_flip: 1, chime: 1, area_up: 1, area_down: 1,
    celebrate: 1
  },
  ...
  prepareGoatAudio() {
    ...
    /* ------------------------------------------------------------- the bed --
       One looping track under everything. It is mixed deliberately quiet and
       it DUCKS: every time the game says something - a measuring tick, a
       success, a bleat, a line of voice-over - the bed drops out of the way
       and eases back. A bed that never moves is the thing that ends up
       covering the sounds that actually carry information. */
    this._bgm = make('Neon Horizon.mp3', this.BGM.vol, true);

    this._unlockGoatAudio = () => {
      this._goatAudioUnlocked = true;
      this.ac();
      this.startBgm();                 // browsers need a gesture before audio
  ...
  startBgm() {
    const b = this._bgm;
    if (!b || this.options.music === false || this.options.audio === false) return;
    if (!b.el.paused) return;
    b.el.volume = 0;
    const p = b.el.play();
    if (p && p.catch) p.catch(() => {});          // blocked until a gesture
    this.fadeGoatAudio(b, this.BGM.vol, this.BGM.fadeIn, false);
  },
  stopBgm() {
    if (this._bgm) this.fadeGoatAudio(this._bgm, 0, 400, true);
  },
  /* Step the bed aside, then bring it back. Re-ducking while already ducked
     just extends the dip rather than stacking, so a run of ticks reads as one
     continuous step-aside instead of a stutter. */
  duckBgm(depth, ms) {
    const b = this._bgm;
    if (!b || b.el.paused || this.options.music === false) return;
    const floor = this.BGM.vol * depth;
    if (b.el.volume > floor) this.fadeGoatAudio(b, floor, 90, false);
    clearTimeout(this._bgmUp);
    this._bgmUp = this.afterKeep(ms, () => {
      if (this._bgm && !this._bgm.el.paused) this.fadeGoatAudio(this._bgm, this.BGM.vol, 620, false);
    });
  },
  ...
  sfx(kind) {
    ...
    if (this.BGM_DUCK[kind]) this.duckBgm(this.BGM.duck, this.BGM.duckMs);
    ...
    switch (kind) {
      ...
      /* The celebration. Spread across the jump rather than stacked on one
         frame: a lift, a twinkle at the peak, a soft settle on landing. */
      case 'cheer_jump':         this.rasp(300, 900 * v, 0.14, 0.05, 1.6); break;
      case 'cheer_spark':        [1319, 1760, 2093].forEach((f, i) => this.after(i * 55, () => this.pluck(f, 0.07, 0.34))); break;
      /* One small bell at the peak of the jump - her own bell, rung once.
         Repeating it would turn a character detail into a jingle. */
