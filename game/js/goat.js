...
Object.assign(FenceTheFarm.prototype, {
  ...
  /* The bounds constrain her FEET, but what must stay inside the fence is her
     whole silhouette. She is GOAT_METRES tall and the art is about 0.77 as
     wide as it is tall, drawn from 0.42 of that width left of her feet - so
     the padding is derived from the sprite, not guessed. That is what stops
     her shoulder crossing a side rail or her hooves poking through the near
     one at any pasture shape. */
  penBounds(p, L, W) {
    const sprW = this.GOAT_METRES * 0.767;         // idle-0.png is 230 x 300
    // Clearing the rail LINE is not enough: a post is wider than its rail and
    // its art rises out of the ground, so a goat that merely clears the line
    // still ends up standing behind a post. Give her, on each side, half her
    // own silhouette plus half a post plus a little air.
    const post = 212 / 360, air = 0.16;            // post width, in cells
    const padL = sprW * 0.42 + post / 2 + air;
    const padR = sprW * 0.58 + post / 2 + air;
    // The near posts rise about a post-height above the bottom edge, so she
    // has to stand well clear of it or they cut across her legs.
    const padT = 0.55, padB = 1.0;
    const x0 = p.ax + padL * p.cell, x1 = p.ax + L * p.cell - padR * p.cell;
    const y0 = p.ay + padT * p.cell, y1 = p.ay + W * p.cell - padB * p.cell;
    // A pen one metre deep cannot hold a 1.35 m goat cleanly. Rather than
    // centre her - which leaves the near posts cutting across her legs -
    // stand her at the BACK, so the near fence simply passes in front.
    const cx = p.ax + L * p.cell / 2, back = p.ay + padT * p.cell;
    if (x1 < x0 && y1 < y0) return [cx, cx, back, back];
    if (x1 < x0) return [cx, cx, y0, y1];
    if (y1 < y0) return [x0, x1, back, back];
    return [x0, x1, y0, y1];
  },
