...
Object.assign(FenceTheFarm.prototype, {
  ...
  levelPlank() {
    ...
    switch (this.mechanic()) {
      ...
      case 'record':
        return this.plankSay(this.t(this.lv.beaten ? 'instruction.push' : 'instruction.record'));
      case 'misconception':
  ...
  tut8() {
    ...
    this.vo('vo.nice');
    this.after(this.noMotion() ? 0 : 1100, () => this.tutBeat(9));
  },
  ...
  recordBegin() {
    this.el['record-val'].textContent = this.lv.record + ' m²';
    this.fitSign(this.el['record-board']);
    this.el['record-cap'].textContent = this.t('record.title');
    this.el.record.classList.add('ftf-in');
    this.sfx('fence_post_rise');
    this.plankDrop(this.t('instruction.record'));
    this.vo('vo.record');
  ...
  recordStep(area, isBest) {
    ...
    if (area > this.lv.record && !this.lv.beaten) {
      ...
      this.el.record.classList.add('ftf-beat');
      this.after(280, () => { this.el['record-cap'].textContent = this.t('record.new'); });
      this.el['record-val'].textContent = area + ' m²';
      this.fitSign(this.el['record-board']);
      this.sfx('record_break');
      this.setGoat('happy');
      // The level does NOT end here. The goal simply opens up.
      this.plankSay(this.t('instruction.push'));
      return;
    }
    if (this.lv.beaten && area > this.lv.record) {
      this.lv.record = area;
      this.el['record-val'].textContent = area + ' m²';
      this.fitSign(this.el['record-board']);
      if (isBest) this.bump(this.el['record-val'], 'ftf-pop', 300);
    }
    if (isBest) this.musicTier(this.tierFor(area));
  },
  recordSucceed() {
    this.el["record-val"].textContent = (this.g.L * this.g.W) + " m²";
    this.fitSign(this.el["record-board"]);
    this.el.record.classList.add("ftf-flag");
    this.sfx("record_success");
    this.after(620, () => this.plankSay(this.t("success.r2")));
    // Then the same recap every farm ends with.
    this.after(1500, () => {
      const r = this.round(), g = this.g;
