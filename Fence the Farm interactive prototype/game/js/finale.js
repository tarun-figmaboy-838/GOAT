...
Object.assign(FenceTheFarm.prototype, {
  ...
  compareBuilds(A, B, perimeter, opts) {
    ...
    const GAP = 150, SPAN = 1180, GROUND = 500, DEPTH = 288;
    ...
    this.el['fin-a-lbl'].style.left = Math.round(ax + wA / 2 - 150) + 'px';
    this.el['fin-b-lbl'].style.left = Math.round(bx + wB / 2 - 150) + 'px';
    this.el["fin-a-lbl"].style.top = this.el["fin-b-lbl"].style.top = (GROUND + 14) + "px";
    // In the recap the way on sits between the two labels, not over one.
  ...
  finaleTrace(perimeter) {
    ...
    const set = (el, d) => {
      const p = this.pens[d.pen];
      el.setAttribute('x', String(p.ax));
      el.setAttribute('y', String(p.ay));
      el.setAttribute('width', String(d.L * C.cell));
      el.setAttribute('height', String(d.W * C.cell));
      el.setAttribute('pathLength', String(per));
      el.style.strokeDasharray = per;
      el.style.strokeDashoffset = per;
      el.style.transition = 'none';
    };
    set(this.el['trace-a'], { pen: 'finA', L: this.finA.L, W: this.finA.W });
    set(this.el['trace-b'], { pen: 'finB', L: this.finB.L, W: this.finB.W });
    this.el.trace.style.opacity = '1';

    const dur = this.noMotion() ? 0 : 1900;
    this.after(30, () => {
      [this.el['trace-a'], this.el['trace-b']].forEach(el => {
  ...
  finaleTraceOne(id, p, L, W, dur) {
    const el = this.el[id], per = 2 * (L + W);
    this.el['trace-b'].setAttribute('width', '0');
    el.setAttribute('x', String(p.ax)); el.setAttribute('y', String(p.ay));
    el.setAttribute('width', String(L * p.cell)); el.setAttribute('height', String(W * p.cell));
    el.setAttribute('pathLength', String(per));
    el.style.transition = 'none';
    el.style.strokeDasharray = per; el.style.strokeDashoffset = per;
    this.el.trace.style.opacity = '1';
    if (this.noMotion()) { el.style.strokeDashoffset = '0'; return; }
    this.after(30, () => {
      el.style.transition = 'stroke-dashoffset ' + dur + 'ms linear';
      el.style.strokeDashoffset = '0';
    });
  ...
  advancedScreen() {
    ...
    ['adv-1', 'adv-2', 'adv-3', 'adv-4'].forEach(k => { this.el[k].style.opacity = '0'; });
    ...
    const step = (ms, fn) => this.after(this.noMotion() ? 0 : ms, fn);
    step(220,  () => { this.el['adv-1'].style.opacity = '1'; this.sfx('area_up'); });
    step(900,  () => { this.el['adv-2'].style.opacity = '1'; this.sfx('area_up'); });
    step(1600, () => { this.el['adv-3'].style.opacity = '1'; this.sfx('area_up'); });
    step(2300, () => { this.el['curve-peak'].style.opacity = '1'; this.sfx('chime'); });
    step(2900, () => { this.el['adv-4'].style.opacity = '1'; });
  ...
  completeScreen() {
    ...
    this._goatPen = 'main';
    const r = this.ROUNDS[3], cell = 40, L = r.optimum[0], W = r.optimum[1];
    const wait = this.dropPen('main');
    this.after(wait, () => {
      this.applyLayout(r.perimeter, L, W, cell, 640 - L * cell / 2, 196, 3);
